...
helper.log = (type, ...values) => {
    ...
    let error = (new Error()).stack.split("\n")[3]
    if(error.indexOf("(") >= 0) error = error.split("(")[1].split(")")[0].trim()
    console.log(helper.date() + " " + helper.time() + outType +  ("undefined" !== typeof window ? "" : error), ...values.map((value) => helper.stringify(value)))
 ...
 */ 
helper.shuffle = (arr) => {
    helper.assert(Array.isArray(arr),"arr must be an array",...arguments)
    let ar = arr.slice()
