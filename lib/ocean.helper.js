 ...
 * @return undefined
 */
helper.assert = (test, message = null) => {
    if(!test) helper.error(message) && process.exit(-1)
}
...
helper.log = (type, ...values) => {
    ...
    if((type === "debug" || type === "error") && typeof Error !== "undefined") console.log((new Error()).stack.split("\n").slice(1).join("\n"))
}
