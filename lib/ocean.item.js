...
class Item {
    ...
    constructor(content = null) {
        ...
        if(content !== null) {
            helper.info("content", content)
            for(const prop in content) this.Set(prop, content[prop])
        }
    }
     ...
     * @return Item
     */
    Set = (path, value, obj = null) => {
        // helper.todo("save the previous value of the property")
        path = Array.isArray(path) ? path : path.split(".")
        obj = obj === null ? this : obj
        assert(path.length > 0, "cannot set a sub property without a path") 
        assert((Array.isArray(obj) && path[0] !== "*") || (typeof obj === "object" && path[0] !== "*"), "cannot set a value to a non-array/non-object: " + helper.stringify(obj))
    ...
    Get = (path, obj = null) => {
        path = Array.isArray(path) ? path : path.split(".")
        obj = obj || this
        assert(path.length > 0, "cannot get a sub property without a path")
     ...
     */
    Merge = (item) => {
        assert(item instanceof Map, "cannot merge with non Map instance")
