...
Item.New = (content = null) => {
    ...
    let item = {}
    item.__status = content?.__status || "success"
    item.__id = content?.__id || null
    Item.SetModel(item, content?.__model)
    if(item.__id === null) {
        item.__id = Date.now()
        while(item.__id <= Item.lastTimestamp) item.__id = Date.now()
        Item.lastTimestamp = item.__id
        item.__id = Number(item.__id).toString(36).toUpperCase().padStart(12,"0")
        item.__id = [item.__id.substr(0,4),item.__id.substr(4,4), item.__id.substr(8)].join("-")
    }
...
Item.Set = (item, path, value = null, obj = null) => {
    ...
    if(item.__status !== "success") return item
 ...
 * @return Item
*/
Item.SetName = (item, name, nametype = "short") => {
