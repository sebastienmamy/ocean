...
class Store {
    ...
    static Init(type, name, path) {
        Store.self = new Item({type,name, path})
    ...
    static Push = (item) => {
        ...
        Store.self.Set(["items", item.__id], item)
        return Store.Index(item.__id, item)
    }
     ...
     * @return Item
     */
    static Identity = (name = null, nametype = "short") => {
        let identity = Store.New("Identity")
        if(name !== null) {
